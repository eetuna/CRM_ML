...
#include "CRM.hpp"
#include "CRMDYN.hpp"
#include "CRMDYN_DYNNLEquation_autodiff.hpp"
...
public:
    ...
    py::dict linearize_full_seed_action_from_seed_implicit(
        ...
        // Jxx uses autodiff through the templated residual math path; fallback to FD if that fails.
        Eigen::MatrixXd Jxx(x_dim, x_dim);
        Eigen::MatrixXd Jxx_fd;
        bool have_ad_jxx = false;
        try {
            // Build DYNNLEParams once at (curr0, seed0) and evaluate Jxx = dF/dx at x*.
            py::array_t<double> vA, wA, pA, RA, xfA, mLA, nLA;
            ...
            Eigen::VectorXd Fad;
            Eigen::MatrixXd Jad = DYNNLEquationJacobianAD(x_star_scaled, DYNNLEParams, &Fad);
