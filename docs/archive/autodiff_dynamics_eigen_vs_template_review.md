...
- `data/output/bench_eigen.jsonl`, `data/output/bench_template.jsonl`
- `data/output/bench_eigen.log`, `data/output/bench_template.log`
...
python3 scripts/dynamics_fk_validation/compare_circle_linearization_report.py \
  --currents-csv data/output/ramp_then_circle1_currents_hold1.csv \
  --out-json data/output/paper_circle_report_hold1_k100.json \
  --k 100 --dirs 5 --du-sigma 1e-5 --seed-sigma 0.0 --seed 0

# Hold2: circle has 400 rows (start=240)
python3 scripts/dynamics_fk_validation/compare_circle_linearization_report.py \
  --currents-csv data/output/ramp_then_circle1_currents_hold2.csv \
  --out-json data/output/paper_circle_report_hold2_k200.json \
  --k 200 --dirs 5 --du-sigma 1e-5 --seed-sigma 0.0 --seed 0
```

Outputs:
- `data/output/paper_circle_report_hold1_k100.json` (+ per-module `.eigen.json` / `.template.json`)
- `data/output/paper_circle_report_hold2_k200.json` (+ per-module `.eigen.json` / `.template.json`)
...
We validated on the real “ramp_then_circle1” trajectories (from `data/output/RUN_INSTRUCTIONS_currents_y40_r10.md`) using the paper-style report in §2.5.
...
- Hold1 (`k=100`, `dirs=5`, `du_sigma=1e-5`, `seed_sigma=0`)
  ...
  - report: `data/output/paper_circle_report_hold1_k100.json`
- Hold2 (`k=200`, `dirs=5`, `du_sigma=1e-5`, `seed_sigma=0`)
  ...
  - report: `data/output/paper_circle_report_hold2_k200.json`
