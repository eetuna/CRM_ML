...
// Forward declaration of template type aliases (needed for DYNNLEqnParamsAD)
...
// Forward declaration of interpolate_fcum
template <typename Scalar>
Vec3<Scalar> interpolate_fcum(const DYNNLEqnParams& Params, const double s);
...
//   - K_diag[3]: diagonal of stiffness matrix (simplified to diagonal for now)
//   - ustar[3]: rest curvature
//   - actMass[1]: actuator mass
//   - MagMoment[3]: magnetic moment vector
//   Total: 16 parameters
//
// Parameter ordering in theta vector:
//   [0:6)   = damping
//   [6:9)   = K_diag (diagonal elements)
//   [9:12)  = ustar
//   [12:13) = actMass
//   [13:16) = MagMoment
// ============================================================================
...
// Offsets into the theta parameter vector
...
// Shadow parameter struct for autodiff-enabled parameters
// This struct holds learnable parameters as templated Scalar types,
// while non-learnable parameters remain as double from the base DYNNLEqnParams.
template <typename Scalar>
struct DYNNLEqnParamsAD {
    // Learnable parameters (templated on Scalar = autodiff::real for gradients)
    Eigen::Matrix<Scalar, 6, 1> damping;           // [6] linear + angular damping
    Eigen::Matrix<Scalar, 3, 1> K_diag;            // [3] diagonal of stiffness matrix
    Eigen::Matrix<Scalar, 3, 1> ustar;             // [3] rest curvature
    Scalar actMass;                                // [1] actuator mass
    Eigen::Matrix<Scalar, 3, 1> MagMoment;         // [3] magnetic moment

    // Non-learnable parameters (fixed as double, copied from base)
    Eigen::Vector3d B0;
    Eigen::Vector3d g;
    Eigen::Matrix3d actInertia;  // Assumed non-learnable for now
    double DELTA_T;

    // Pointer back to original params for structural/geometry data
    const DYNNLEqnParams* base_params;

    // Construct full K and Kinv matrices from diagonal
    ...
    // Get muhat (skew-symmetric matrix of magnetic moment)
...
// Pack learnable parameters from DYNNLEqnParams into a flat vector
inline Eigen::VectorXd packLearnableParams(const DYNNLEqnParams& params, int flex_seg_index = 0) {
    ...
    // 1. damping[6] (Linear + Angular)
    // Accessing directly from the fixed-size array in the base struct
    for (int i = 0; i < 6; ++i) {
        theta(THETA_OFFSET_DAMPING + i) = params.damping[0][i];
    }

    // 2. K_diag[3] (Stiffness Diagonal)
    if (params.no_flex_seg > flex_seg_index) {
        // params.K is double (*K)[9]
        theta(THETA_OFFSET_K_DIAG + 0) = params.K[flex_seg_index][0];
        theta(THETA_OFFSET_K_DIAG + 1) = params.K[flex_seg_index][4];
        theta(THETA_OFFSET_K_DIAG + 2) = params.K[flex_seg_index][8];
    }

    // 3. ustar[3] (Rest Curvature)
    if (params.no_flex_seg > flex_seg_index) {
        for (int i = 0; i < 3; ++i) {
    ...
    // 5. MagMoment[3] (Magnetic Moment Product)
    for (int i = 0; i < 3; ++i) {
...
// Get parameter names for debugging/reporting
...
// Unpack theta vector into AD parameter struct
template <typename Scalar>
    ...
    const DYNNLEqnParams& base_params)
{
    ...
    // Unpack learnable parameters from the differentiable theta vector
    for (int i = 0; i < 6; ++i) {
    ...
    // Copy non-learnable (fixed) parameters from the legacy struct
    for (int i = 0; i < 3; ++i) {
        ad_params.B0(i) = base_params.B0[i];
        ad_params.g(i) = base_params.g[i];
    }
    
    // Copy actuator inertia (assumed fixed for now)
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
...
// Note: Vec3, Vec6, Mat3 templates are already declared at the top of the namespace
...
inline void CoilIntegrand(const Vec6<Scalar>& twist,
                          ...
                          const Eigen::Matrix<double, 6, 1>& damping,
                          const Eigen::Vector3d& B0,
                          const Eigen::Matrix3d& muhat,
                          ...
                          Vec6<Scalar>& twistdot)
{
    ...
    const Vec3<Scalar> Tb = muhat.template cast<Scalar>() * RscTB0;
...
// ============================================================================
// AD-Parameter-Enabled Versions (for parameter gradients)
// These versions take DYNNLEqnParamsAD<Scalar> instead of double parameters,
// enabling autodiff to trace gradients through the physical parameters.
// ============================================================================

// CoilIntegrand with templated learnable parameters
template <typename Scalar>
inline void CoilIntegrandAD(const Vec6<Scalar>& twist,
                            const Vec3<Scalar>& n_L,
                            const Eigen::Vector3d& g,
                            const Mat3<Scalar>& R,
                            ...
                            Vec6<Scalar>& twistdot)
{
    ...
    Vec3<Scalar> vdot;
    for (int i = 0; i < 3; ++i) {
        const Scalar damp_v = damping(i) * v(i);
        vdot(i) = RTg(i) - n_L(i) / actMass - w_v(i) - damp_v;
    }
    ...
    Vec3<Scalar> residual_w;
    for (int i = 0; i < 3; ++i) {
        const Scalar damp_w = damping(3 + i) * w(i);
        residual_w(i) = (tau(i) - w_inertia_w(i) - damp_w);
    }
...
// CoilDynamics with templated learnable parameters
template <typename Scalar>
inline void CoilDynamicsAD(const Vec6<Scalar>& v0w0,
                           const Vec3<Scalar>& p0,
                           const Mat3<Scalar>& R0,
                           const Vec3<Scalar>& n_L,
                           const Eigen::Vector3d& g,
                           ...
                           Vec6<Scalar>& out_xdot_n)
{
    ...
    for (int idx = 0; idx < N; ++idx) {
        CoilIntegrandAD(twist_n, n_L, g, R_n, actMass, actInertia, damping, B0, muhat, m_L, xdot_n);
        out_xdot_n = xdot_n;
        ...
        if (idx < 3) {
            ...
            DYNSE3_TimeSpace(R_n, p_n, h * Scalar(0.5), twist_n, R_half, p_half);
            Vec6<Scalar> xdot_half;
            CoilIntegrandAD(twist_half, n_L, g, R_half, actMass, actInertia, damping, B0, muhat, m_L, xdot_half);
            twist_np1 = twist_n + h * xdot_half;
            DYNSE3_TimeSpace(R_n, p_n, h, twist_half, R_np1, p_np1);
        } else {
            ...
            Vec6<Scalar> xdot_hat;
            CoilIntegrandAD(twist_hat, n_L, g, R_hat, actMass, actInertia, damping, B0, muhat, m_L, xdot_hat);
...
// CRMIntegrand_dyn with templated K, Kinv, ustar
template <typename Scalar>
...
// CRMFlexible_IVP_Back with templated K, Kinv, ustar
template <typename Scalar>
                                     ...
                                     const Vec3<Scalar>& nL_local)
{
    ...
    const Eigen::Map<const Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> Kd(Params.K[fsegno]);
    const Eigen::Map<const Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> Kdinv(Params.Kinv[fsegno]);
    const Eigen::Map<const Eigen::Matrix<double, 3, 1>> ustar_d(Params.ustar[fsegno]);

    const Mat3<Scalar> K = Kd.template cast<Scalar>();
    const Mat3<Scalar> Kinv = Kdinv.template cast<Scalar>();
    const Vec3<Scalar> ustar = ustar_d.template cast<Scalar>();
                                                                              ...
                                                                              const DYNNLEqnParams* ParamsPtr)
{
    ...
    const double actMass = Params.ActMass[0];
    ...
    for (int i = 0; i < 6; ++i) damping(i) = Params.damping[0][i];

    // Magnetic moment hat.
    Eigen::Vector3d mu;
    for (int i = 0; i < 3; ++i) mu(i) = Params.MagMoment[0][i];
    Eigen::Matrix3d muhat;
    muhat << 0.0, -mu(2), mu(1),
        mu(2), 0.0, -mu(0),
        -mu(1), mu(0), 0.0;
    ...
    for (int segi = NUM_SEGMENTS - 1; segi >= 0; --segi) {
        if (segi % 2 == 0) {
            ...
            const Eigen::Map<const Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> Kinv_d(Params.Kinv[fsegi]);
            const Eigen::Map<const Eigen::Matrix<double, 3, 1>> ustar_d(Params.ustar[fsegi]);
            const Vec3<Scalar> ustar = ustar_d.template cast<Scalar>();
            const Mat3<Scalar> Kinv = Kinv_d.template cast<Scalar>();
            ...
            // tau = K * (u_tau - ustar)
            const Eigen::Map<const Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> K_d(Params.K[fsegi]);
            const Mat3<Scalar> K = K_d.template cast<Scalar>();
            tau = K * (u_tau - ustar);
...
// ============================================================================
// Residual function with templated parameters for parameter gradients
// This version takes both x_scaled (solver state) and theta (learnable params)
// as autodiff variables, enabling ∂F/∂θ computation.
// ============================================================================
template <typename Scalar>
    ...
    const DYNNLEqnParams* ParamsPtr)
{
    ...
    // Unpack theta into AD parameter struct
    ...
    // start from tip state Params.xf
    Vec3<Scalar> p_t;
    Mat3<Scalar> R_t;
    for (int i = 0; i < 3; ++i) p_t(i) = Scalar(Params.xf[i]);
    for (int r = 0; r < 3; ++r) for (int c = 0; c < 3; ++c) R_t(r, c) = Scalar(Params.xf[3 + r * 3 + c]);
    ...
    // Get learnable params from AD struct
    ...
    // Get K, Kinv, ustar from AD params
    ...
    for (int segi = NUM_SEGMENTS - 1; segi >= 0; --segi) {
        if (segi % 2 == 0) {
            ...
            CoilDynamicsAD(vw_coil0, p_coil0, R_coil0, net_nL, ad_params.g,
    ...
    // Compute residual against root config
    Vec3<Scalar> res_p = p_f - p_d;
    Vec3<Scalar> res_r;
    for (int k = 0; k < 3; ++k) {
        const Vec3<Scalar> diff = R_f.col(k) - R_d.col(k);
...
// Computes the Jacobian of the dynamics residual with respect to learnable
// physical parameters (damping, stiffness, etc.)
//
// Returns: (NUM_DYN_RESIDUAL x NUM_LEARNABLE_PARAMS) matrix
// ============================================================================
    ...
    Eigen::VectorXd* out_theta = nullptr)
{
    ...
    // Pack current parameters into theta vector (doubles)
    Eigen::VectorXd theta_d = dynnl_ad_eigen::packLearnableParams(Params, 0);

    // Convert x (fixed) and theta (differentiable) to autodiff types
    ...
    // Compute Jacobian w.r.t. theta
    VectorXreal y_ad;
    Eigen::MatrixXd J_theta;

    // We MUST pass theta_ directly to the residual function to allow autodiff to track gradients.
    // The lambda captures x_ad and Params by reference.
...
// Combined Jacobian: returns both ∂F/∂x and ∂F/∂θ
// This is useful when you need both state and parameter sensitivities
inline void DYNNLEquationFullJacobiansEigenAD(
    const Eigen::VectorXd& x_scaled,
    DYNNLEqnParams& Params,
    Eigen::MatrixXd& J_x,           // out: (NUM_DYN_RESIDUAL x NUM_DYN_RESIDUAL)
    Eigen::MatrixXd& J_theta,       // out: (NUM_DYN_RESIDUAL x NUM_LEARNABLE_PARAMS)
    Eigen::VectorXd* out_residual = nullptr,
    Eigen::VectorXd* out_theta = nullptr)
{
    using autodiff::VectorXreal;
